...
import { User, Announcement, HogRaisingState, IgpChoreLog, Member } from '../types';
import { 
  ...
  Megaphone, Search, Bell, ChevronDown, ChevronUp, PiggyBank
} from 'lucide-react';
...
export default function MemberDashboard({ 
  ...
  const [activeTab, setActiveTab] = useState<'profile' | 'announcements' | 'hog-raising'>('profile');
  ...
  return (
    <div id="member-dashboard-root" className="space-y-6 text-[#2D3A22]">
      ...
      <div className="flex border-b border-[#D5CFC1] no-print gap-1 select-none flex-wrap">
        <button
          id="member-profile-tab-btn"
          onClick={() => setActiveTab('profile')}
          className={`px-5 py-3.5 text-sm sm:text-base font-black transition-all flex items-center gap-2 border-b-2 cursor-pointer ${
            activeTab === 'profile'
              ? 'border-[#1B4332] text-[#1B4332] bg-[#D8F3DC] rounded-t-2xl'
              : 'border-transparent text-[#2D3A22] hover:text-[#1B4332] hover:bg-[#FAF8F5]'
          }`}
        >
          <UserIcon className="w-5 h-5 text-[#1B4332]" />
        ...
        >
          <PiggyBank className="w-5 h-5 text-[#1B4332]" />
        ...
        <button
          ...
          className={`px-5 py-3.5 text-sm sm:text-base font-black transition-all flex items-center gap-2 border-b-2 cursor-pointer relative ${
        ...
        >
          ...
          {announcements.filter(ann => !readAnnouncements.includes(ann.id)).length > 0 && (
            <span className="bg-rose-600 text-white font-black text-xs px-2.5 py-0.5 rounded-full flex items-center justify-center animate-pulse min-w-5 h-5">
                  ...
                  <div className="bg-gradient-to-br from-[#1B4332] via-[#24543F] to-[#143326] border-2 border-[#D8F3DC]/30 rounded-2xl p-4 relative overflow-hidden h-[54mm] flex flex-col justify-between shadow-md text-white">
                    ...
                    <div className="flex items-start justify-between border-b border-white/10 pb-1.5 relative z-10">
                      ...
                      <div className="text-right">
                        ...
                        <span className="text-[5.5px] text-[#D8F3DC] font-black uppercase block leading-none">REGULAR MEMBER</span>
                      </div>
                    </div>
