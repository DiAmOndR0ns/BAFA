...
import { RotateCw, Trash2, Cloud, CloudOff, RefreshCw, Layers, CheckCircle2, ArrowRight } from 'lucide-react';
      ...
      {queue.length > 0 ? (
        <div className="space-y-3">
          <div className="bg-amber-500/5 border border-amber-500/20 rounded-xl p-3 text-xs text-amber-300 leading-relaxed flex items-start gap-2">
            ...
            <div>
              <span className="font-bold">Queue Active:</span> {queue.length} operation(s) are stored in client state. When you toggle <strong>Simulate Online</strong> above and click <strong>Sync Queued Data</strong>, these items will automatically flush into the active record system with real timestamp resolution.
            </div>
          </div>
        ...
        <div className="py-8 text-center text-slate-500 space-y-2 text-sm">
          ...
          <p className="text-xs text-slate-500 max-w-sm mx-auto">
            All offline transactions, registrations, and logs have been synced correctly. Toggle "Simulate Offline" above and perform operations to see them queue up!
          </p>
        </div>
      )}
    </div>
